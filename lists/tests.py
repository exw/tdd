...
class HomePageTest(TestCase):
    ...
    def test_home_page_displays_all_list_items(self):
        Item.objects.create(text='testItem 1')
        Item.objects.create(text='testItem 2')

        request = HttpRequest()
        response = home_page(request)

        self.assertIn('testItem 1', response.content.decode())
        self.assertIn('testItem 2', response.content.decode())
